...
3. Where FR-71 puts different cases of the same PRD Appendix defect in different bands, the
   *detector* (`cell_defects.py`) splits the finding code by shape at detection time, so the
   ...
   - `CODE_CELL_NOT_TEXT` (a numeric code cell; the digits are intact, so coercion to a
     string is deterministic) vs. `CODE_CELL_INVALID_TYPE` (a date, boolean, formula or
     error code cell; there is no value to recover, only a wrong one to report - blocks).
   - `UNRECOGNISED_LAYOUT` (the code column isn't resolved - whether the sheet resolves
     some other SPIA columns or none at all - blocks) vs. `SHEET_NOT_SPIA_DATA` (resolves
     no SPIA column *and* is named in a small, explicit allowlist of FR-63's documented
     non-SPIA-data worksheets - informational, does not block).
   - The `UNRECOGNISED_LAYOUT`/`SHEET_NOT_SPIA_DATA` split is gated on the sheet's *name*,
     not merely on resolving zero column roles: resolving zero roles is not itself evidence
     a sheet isn't SPIA data - a genuine data sheet whose header row has drifted entirely
     (for example, an inserted banner row above the real FR-63 headers) produces the
     identical signal, and must still block rather than being waved through because no
...
| Gate `SHEET_NOT_SPIA_DATA` on a row-count or column-count threshold instead of a sheet-name allowlist | A threshold is guessable in both directions: a real `Rev History`-shaped sheet could exceed it (a long release-notes history), and a drifted SPIA sheet with few rows could fall under it - either way the gate fires on the wrong signal. FR-63 documents exactly one non-SPIA-data worksheet, by name; an allowlist keyed on that name is no more fragile than the header-text matching `column_role` already does; it is just fragile in a way that fails safe (an unrecognised name blocks) rather than one that fails open (an unrecognised shape passes silently). |
| Band `CODE_CELL_NOT_TEXT` as `data-defect` for every non-text cell type, rather than splitting it | Simpler, but wrong in the other direction: the numeric case (39 of 50 sample rows, PRD Appendix A.2) is exactly FR-71's own "type coercion of codes to string" auto-correctable example, and banding it as a blocking data defect would abort every real seed run before P0-9 even exists to fix it. |
...
- P0-3 emits nine finding codes instead of P0-2's original five. `docs/operations/runbooks/transform.md`'s
  finding-code table and this decision are now the two places that must stay in sync if a
  future PRD change adds another.
- The sheet-name allowlist (`cell_defects._NON_SPIA_DATA_SHEET_NAMES`) is one more place that
  must be extended if a future FR-63 revision names a second non-SPIA-data worksheet - it
  will not happen automatically the way a purely structural signal would, and that is the
  point: a name has to be added deliberately, not inferred.
