...
| `transaction.ts` | The in-flight `{state, code_verifier, nonce, redirect}`, in `sessionStorage`, **read-and-deleted in one step** |
...
| `auth-context.tsx` | The session: tokens in memory, renewal, sign-in/out, callback completion |
| `session.ts` | The context object, its value type, and `useAuth` |
...
- **The `state` check happens before the code is sent anywhere**, and consumes the
  transaction. A replayed callback URL therefore fails without a network call.
