...
| H-06 | Silent content change | A modification to catalogue content is undetected or unattributable | NFR-08–NFR-10 append-only, hash-chained audit log | Privilege half implemented (issue #33): `audit_event` table with an INSERT/SELECT-only grant, proven by refusal tests, not yet a real write path or the hash chain |
