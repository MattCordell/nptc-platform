...
|---|---|---|---|---|
| H-01 | Wrong test ordered | A synonym collides with another entry's preferred term; a requester selects the wrong item | FR-05 error-severity collision detection blocks the save | Mitigation implemented (issue #49): `nptc.catalogue.collisions.assert_no_error_collisions`, checked before any row is constructed in every write path of `nptc.catalogue.designations`/`entries` |
| H-02 | Wrong test ordered | An ambiguous synonym maps to multiple entries differing only by specimen | FR-05 warning-severity detection plus mandatory specimen display in search results | Collision detection half implemented (issue #49): `nptc.catalogue.collisions.warning_collisions`/`acknowledge_collision`. The mandatory specimen display in search results is not yet built (#138) |
...
| H-05 | Order rejected downstream, or ordered against an inactivated concept | The catalogue references a SNOMED CT code that has since been inactivated | FR-45–FR-47 dual-edition validation, FR-56 publication gate | Mitigation designed, not yet implemented. A partial, earlier mitigation is implemented (issue #240, FR-26): `GET /api/v1/terminology/concepts/{code}` shows an editor the concept's live active status at the moment they bind a code, before the FR-45–FR-47 sweep or the FR-56 gate ever run against it — this catches a code that is *already* inactive at bind time, not one inactivated afterwards, which is still only caught by the sweep/gate above |
